...
Сервис принимает команды через TCP сокет или WebSocket и перенаправляет их на HTTP эндпоинты целевого API. Это позволяет отвязать логику сокет клиента от конкретной реализации приложения и управлять любым сервисом через единый сокет интерфейс. Также поддерживает интерактивный SSH-терминал и PTY доступ.
